...
//modified by Alex Simes to specifically work for our easyview application

Ext.define("easyview.plugins.printer.Printer", {
    ...
    statics: {
                ...
                for (var key in item.data) {
                    ...
                    Ext.each(columns, function(column, col) {
                        ...
                        if (column && column.dataIndex == key) {
                            ...
                            var varName = Ext.String.createVarName(String(column.dataIndex));
                            convertedData[String(varName)] = value;
                            found = true;
			...
			//console.log(column);
                    if ( column ) {
                        if ( !Ext.isEmpty(column.dataIndex) &&
                             !column.hidden                 &&
                             !isGrouped && (column.xtype!='textfield')){
                            	clearColumns.push(column);
				//console.log("pushed column, standard");
			
			} else if ( column.xtype === 'textfield' && !column.ownerCt.hidden ){
				//console.log("adding name column");
				//console.log(column);
				column.dataIndex = 'name';
				column.text = "Name";
                            clearColumns.push(column);
				//console.log("pushed column, textfield");
	
                        } else if ( column.xtype === 'rownumberer'){
                            column.text = 'Row';
                            clearColumns.push(column);
				//console.log("pushed column, rownumberer");
                        } else if ( column.xtype === 'templatecolumn'){
                            clearColumns.push(column);
				//console.log("pushed column, template column");
                        } else if ( isGrouped && column.dataIndex !== groupField ){
                            clearColumns.push(column);
				//console.log("pushed column, grouped");
                        }
                    }
                }
            );
		//console.log(columns);
            ...
            if (this.stylesheetPath === null) {
                var scriptPath = Ext.Loader.getPath('easyview.plugins.printer.Printer');
         ...
         */
        stylesheetPath: 'resources/gridPrinterCss/print.css',
