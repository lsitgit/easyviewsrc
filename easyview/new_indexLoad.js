 ...
 * */
	...
	//grade_items defined in bottom of index.php 
		...
		var feedback_name = grade_items[i]['feedback'];
			...
			image = "<img id='"+grade_items[i]['gid']+"' class='header-icon' style='vertical-align:middle;margin-bottom:4px;' src='resources/manual_item.svg'/>";
				...
				itemId:(grade_items[i]['type']+i+grade_items[i]['cat_name']).replace(/["'()%\[\]{}\\=+\s&#@\^,\.!\$\*-]/g,''),
					...
					if(easygrid.columns[colIdx]['text']!=""){
						...
						//console.log(feedback_name+": "+data.get(feedback_name));
    						metaData.tdAttr = 'data-qtip="Feedback:'+data.get(feedback_name)+'"';

							/* + data.get('first').replace(/\\'/g,"'")+" "+data.get('last').replace(/\\'/g,"'")+
							"<p>"/*+grade_name+", Score: "+new_value+"</strong><p>"+data.get(feedback_name) + '"'*/
						//if(feedback_name=="427feedback"){ console.log("Midterm tooltip: "+metaData.tdAttr);}
					}
					return new_value;
                                },
                                summaryRenderer: function(value, summaryData, dataIndex) {
